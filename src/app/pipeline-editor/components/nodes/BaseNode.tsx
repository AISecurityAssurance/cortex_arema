...
interface BaseNodeProps {
  ...
  hoveredPort?: { nodeId: string; portType: 'input' | 'output'; portName: string } | null;
  onMouseDown: (e: React.MouseEvent) => void;
  onPortMouseDown: (e: React.MouseEvent, nodeId: string, portType: 'input' | 'output', portName: string) => void;
  onPortMouseUp: (e: React.MouseEvent, nodeId: string, portType: 'input' | 'output', portName: string) => void;
  onPortMouseEnter: (nodeId: string, portType: 'input' | 'output', portName: string) => void;
  ...
  const getNodeDescription = () => {
    switch (node.type) {
      ...
      case 'analysis-stpa-sec':
        return `Model: ${(node as any).config.modelId?.split('.')[2] || 'Not selected'}`;
      case 'output-results':
  ...
  return (
    ...
    >
      <div className={`node-header ${nodeTypeClass}`}>
        ...
        <button className="node-config-btn" onClick={(e) => e.stopPropagation()}>
          ⚙
        </button>
      </div>
      ...
      <div className="node-body">
        ...
        {executionState && executionState.status === 'running' && (
          <div className="node-status">
            Running... {executionState.progress ? `${Math.round(executionState.progress)}%` : ''}
          </div>
        )}
        ...
        {'inputs' in node && (
          <div 
            className={`port input ${
              isConnecting && connectionStart && connectionStart.nodeId !== node.id ? 'connectable' : ''
            } ${
              hoveredPort?.nodeId === node.id && hoveredPort?.portType === 'input' ? 'hovered' : ''
            }`}
            onMouseDown={(e) => onPortMouseDown(e, node.id, 'input', (node as any).inputs[0])}
            onMouseUp={(e) => onPortMouseUp(e, node.id, 'input', (node as any).inputs[0])}
            onMouseEnter={() => onPortMouseEnter(node.id, 'input', (node as any).inputs[0])}
        ...
        {'outputs' in node && (
          <div 
            className={`port output ${
              hoveredPort?.nodeId === node.id && hoveredPort?.portType === 'output' ? 'hovered' : ''
            }`}
            onMouseDown={(e) => onPortMouseDown(e, node.id, 'output', (node as any).outputs[0])}
            onMouseUp={(e) => onPortMouseUp(e, node.id, 'output', (node as any).outputs[0])}
            onMouseEnter={() => onPortMouseEnter(node.id, 'output', (node as any).outputs[0])}
            onMouseLeave={onPortMouseLeave}
